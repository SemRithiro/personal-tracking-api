...
import com.rithiro.personaltracking.models.properties.TokenTimeout;
...
import com.rithiro.personaltracking.models.requests.TokenInfoRequest;
...
public class AuthenticationHelper {
	...
	public TokenResponse login(LoginRequest loginRequest, ClientInfoRequest clientInfoRequest) {
		...
		Auth authUser = userService.findByUsername(loginRequest.getUsername());

		if (authUser != null) {
			accessTokenInfo.setIssuer(ISSUER_ID);
			accessTokenInfo.setTokenId(jwtId);
			accessTokenInfo.setUserId(authUser.getId());
			accessTokenInfo.setSubject(authUser.getAuthId());
	...
	public TokenResponse refreshToken(RefreshTokenRequest refreshTokenRequest, ClientInfoRequest clientInfoRequest) {
		...
		UserResponse authUser = userService.findUserByAuthId(subject);
